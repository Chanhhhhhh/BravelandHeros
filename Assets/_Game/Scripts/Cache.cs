...
public class Cache
{
    ...
    public static Dictionary<Character, IHealth> m_CharacterHealth = new Dictionary<Character, IHealth>();
    public static IHealth GetHealth(Character key)
    {
        if (!m_CharacterHealth.ContainsKey(key))
        {
            m_CharacterHealth[key] = key.GetComponent<IHealth>();
        }

        return m_CharacterHealth[key];
    }
}
